    ...
    fEvent(theEvent)
{
	...
	if (fEvent.GetSimData().GetSimulationMode() == SimData::SimulationMode::eFast) {
		cout << "[INFO] G4HodoscopeSimulator::G4HodoscopeSteppingAction: Running Simulation in Fast mode." << endl;
		cout << "[INFO] G4HodoscopeSimulator::G4HodoscopeSteppingAction: Optical photons will not be propagated!" << endl;
